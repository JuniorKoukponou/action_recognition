...
def load_data():
    dataset = pd.read_csv('data/data_3mdad_keypoints.csv', index_col=None)

    dataset['0'] = dataset['0'].str.replace("[", "")
    dataset['0'] = dataset['0'].str.replace("]", "")
    dataset = pd.concat([dataset['0'].str.split(expand=True,), dataset['1']], axis=1)
    dataset.rename(columns={'1':'28'}, inplace=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_file', type=str, default='./models/lstm_3mdad.h5')
